...
function checkValue(originV, newV, func){
  return originV != undefined ? func(originV, newV) : newV;
}
...
function splitMc(image, config){
  ...
  const images = {};
  const renameByOder = Object.keys(config.mc).length == 1;
  for(let k in frames){
    const {x, y, w, h, } = frames[k];
    const {width, height, offsetX, offsetY, order} = infos[k];
    ...
    try {
      ...
      images[renameByOder ? order : k] = newImage;
    } catch (e) { 
      // console.error("write image file error", config, infos);
      console.error("write image file error", k, x, y, w, h,offsetX, offsetY);
    }
  }
  return images;
}
...
function getMCImagesInfo(config){
  ...
  for(let k in actions){
    ...
    action.frames.forEach(frame=>{
      ...
      const bottom = top + data.h;

      let tmp = tmps[res];
      if(!tmp){
        tmps[res] = tmp = {};
        res2Order.set(res, order ++);
      }
